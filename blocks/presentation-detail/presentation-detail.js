...
function renderDetail(asset, block) {
  ...
  main.className = 'presentation-detail-main';

  const backLink = document.createElement('a');
  backLink.className = 'presentation-detail-back';
  backLink.href = '#';
  ...
  previewWrapper.className = 'presentation-detail-preview';

  const viewerUrl = `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(`${publishHost}${asset.path}`)}`;
  ...
  actionsSection.className = 'presentation-detail-actions';
  const actionsRow = document.createElement('div');
  actionsRow.className = 'presentation-detail-actions-row';
  ...
  sidebar.className = 'presentation-detail-sidebar';
...
export default async function decorate(block) {
  ...
  block.innerHTML = '<div class="presentation-detail-loading">Loading presentation details...</div>';
  ...
  try {
    ...
    block.innerHTML = `<p>Failed to load presentation details: ${err.message}</p>`;
  }
}
